...
2. Cross-List Parents: '_fetch_missing_parents' fetches parent tasks from other lists for accurate rollups.
3. Centralized Math: '_calculate_task_metrics' helper powers ALL reports.
4. Estimation Accuracy: Properly implemented using bottom-up sums.
5. Robust Status Logic: Identifies 'Shipped', 'Release', etc. as DONE.
6. Complete Toolset: Includes all analytics, breakdowns, and risk assessments.
"""
...
def _fetch_missing_parents(all_tasks: List[Dict]) -> List[Dict]:
    """
    Fetch parent tasks that exist in different lists (cross-list subtasks).
    This ensures accurate bottom-up time rollups when subtasks reference
    parents that are not in the fetched list.
    """
    existing_ids = {t["id"] for t in all_tasks}
    missing_parent_ids = set()

    # Find all parent IDs that are not in the current task set
    for t in all_tasks:
    ...
    # Fetch each missing parent
    for pid in missing_parent_ids:
        data, err = _api_call("GET", f"/task/{pid}")
        if data and not err and data.get("id") not in existing_ids:
            existing_ids.add(data["id"])
            extended_tasks.append(data)

            # Also check for grandparent (1 level up) for deep hierarchies
            grandparent_id = data.get("parent")
            if grandparent_id and grandparent_id not in existing_ids:
                gp_data, gp_err = _api_call("GET", f"/task/{grandparent_id}")
                if gp_data and not gp_err and gp_data.get("id") not in existing_ids:
...
def register_pm_analytics_tools(mcp: FastMCP):
    ...
    def get_time_tracking_report(
        ...
        try:
            ...
            all_tasks = _fetch_missing_parents(
                all_tasks
            )  # Include cross-list parents for accurate rollups
    ...
    def get_task_time_breakdown(task_id: str) -> dict:
        ...
        try:
            ...
            all_list_tasks = _fetch_missing_parents(
                all_list_tasks
            )  # Include cross-list parents
