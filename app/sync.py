...
from app.supabase_db import get_employee_id_map, get_existing_task_ids, mark_tasks_deleted, bulk_upsert_tasks, get_all_task_ids, bulk_update_comments
from app.clickup import fetch_all_time_entries_batch, fetch_all_spaces, fetch_assigned_comments_batch, _get, BASE_URL
    ...
    for space in fetch_all_spaces():
        ...
        for folder in _get(f"{BASE_URL}/space/{sid}/folder").get("folders", []):
            for lst in folder.get("lists", []):
                loc[lst["id"]] = {"space_id": sid, "space_name": sname, "folder_id": folder["id"], "folder_name": folder["name"], "list_id": lst["id"], "list_name": lst["name"]}
        for lst in _get(f"{BASE_URL}/space/{sid}/list").get("lists", []):
            loc[lst["id"]] = {"space_id": sid, "space_name": sname, "folder_id": None, "folder_name": "None", "list_id": lst["id"], "list_name": lst["name"]}
    return loc
...
def _get_sprint_points(task):
    ...
    if task.get("points"):
        try:
            ...
            pass
    val = _get_custom_field(task, "sprint points") or _get_custom_field(task, "points") or _get_custom_field(task, "story points")
    if val:
        try:
...
def sync_tasks_to_supabase(tasks, *, full_sync):
    ...
    payloads = []
    for t in tasks:
        tid, status, loc = t["id"], t.get("status", {}), loc_map.get(t.get("list", {}).get("id"), {})
        ...
        employee_ids = [emp_map[str(a["id"])] for a in assignees if str(a.get("id")) in emp_map]

        payloads.append({
            "clickup_task_id": tid,
            "title": t.get("name"),
            "description": t.get("text_content"),
            "type": TYPE_MAP.get(t.get("custom_item_id"), "milestone" if t.get("is_milestone") else t.get("type") or "task"),
            "status": status.get("status", ""),
            "status_type": status.get("type", ""),
            "priority": (t.get("priority") or {}).get("priority"),
            "tags": ", ".join(x["name"] for x in (t.get("tags") or []) if x.get("name")) or None,
            ...
            "followers": ", ".join(w["username"] for w in (t.get("watchers") or []) if w.get("username")) or None,
            **loc,
            ...
            "date_closed": _to_iso(_ms_to_dt(t.get("date_closed"))) if status.get("type") == "closed" else None,
            "start_date": _ms_to_date(t.get("start_date")),
            "due_date": _ms_to_date(t.get("due_date")),
            "time_estimate_minutes": int(t["time_estimate"]) // 60000 if t.get("time_estimate") else None,
