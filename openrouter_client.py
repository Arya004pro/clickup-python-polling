...
def _explicit_report_tool_directive(text: str) -> str | None:
    """
    Detect explicit tool directives embedded in prompts generated by scheduler logic.
    When present, we should narrow the tool surface so the model cannot choose a
    mismatched report tool.
    """
    if not text:
        return None

    lowered = text.lower()
    if re.search(r"\bcall\s+get_space_task_report\s+directly\b", lowered):
        return "get_space_task_report"
    if re.search(r"\bcall\s+get_project_task_report\s+directly\b", lowered):
        return "get_project_task_report"
    if re.search(r"\bcall\s+get_member_task_report\s+directly\b", lowered):
...
class OpenRouterMCPClient:
        ...
        while True:
            try:
                resp = await self.llm.chat.completions.create(
                    ...
                    else (self.openai_tools or None),
                    tool_choice="auto",
                    temperature=0.1,
                    max_tokens=4096,
                )
    ...
    def _tools_for_user_message(self, user_message: str):
        ...
        """
        forced_tool = _explicit_report_tool_directive(user_message)
        if forced_tool:
            narrowed = [
                t
                for t in (self.openai_tools or [])
                if t["function"]["name"] == forced_tool
            ]
            if narrowed:
                print(
                    f"  {col(YELLOW, '!!')} Forced tool mode: {col(DIM, forced_tool)}"
                )
                return narrowed
